  ...
  private class InjectorServiceListener implements ServiceListener {
    ...
    public void serviceChanged(ServiceEvent event) {
      ...
      if (service instanceof ImplementationPicker) {
        if (event.getType() == ServiceEvent.REGISTERED) {
          bindImplementationPicker((ImplementationPicker)service, getServiceProperties(event.getServiceReference()));
        }
        else if (event.getType() == ServiceEvent.UNREGISTERING) {
          unbindImplementationPicker((ImplementationPicker)service, getServiceProperties(event.getServiceReference()));
        }
      }
    }
